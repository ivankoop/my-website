...
const s3Client = new S3Client({
  region: 'us-east-2',
  credentials: {
    accessKeyId: process.env.IK_AWS_ACCESS_KEY,
    secretAccessKey: process.env.IK_AWS_SECRET_KEY,
  },
});

const BUCKET_NAME = 'ivankoop-dev';
...
export default async function gitStats(req, res) {
  try {
    ...
    for (const [dateStr, dayCommits] of Object.entries(
      gitStatsData.data.commits
    )) {
    ...
    const processedData = {
      ...
      summary: {
        ...
        averageCommitsPerDay:
          Object.keys(lastYearCommits).length > 0
            ? Math.round(
                (lastYearCommitCount / Object.keys(lastYearCommits).length) *
                  100
              ) / 100
            : 0,
